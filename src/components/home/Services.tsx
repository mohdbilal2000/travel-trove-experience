...
const ServiceCard = ({ service }) => {
  ...
  return (
    <motion.div variants={item} className="group">
      <Link 
        to={service.link}
        className="h-full block p-6 bg-white rounded-xl shadow-sm border border-border transition-all duration-300 hover:shadow-md hover:border-primary/20 hover:translate-y-[-5px]"
      >
        <div className="w-12 h-12 rounded-md bg-primary/10 flex items-center justify-center mb-4 text-primary transition-all duration-300 group-hover:bg-primary group-hover:text-white">
          <Icon size={24} />
        </div>
